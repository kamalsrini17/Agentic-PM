...
console.log('[ENV] OPENAI_API_KEY loaded:', ((_a = process.env.OPENAI_API_KEY) === null || _a === void 0 ? void 0 : _a.slice(0, 8)) + '...');
...
${JSON.stringify(input, null, 2)}
    `;
            ...
            const match = rawContent.match(/```json\s*({[\s\S]+?})\s*```|({[\s\S]+})/);
            const jsonString = match ? (match[1] || match[2]) : '{}';
            let parsed = {};
            try {
                parsed = JSON.parse(jsonString);
            }
            catch (err) {
                console.error('[ERROR] Failed to parse JSON from OpenAI response:', err);
                console.error('[DEBUG] rawContent:', rawContent);
            }
            const prd = {
                title: `${input.product} PRD`,
                version: '1.0',
...
Only return a valid JSON object with the updated PRD. Do not include markdown or explanation.
`;
            const response = yield openai.chat.completions.create({
                model: 'gpt-4',
                messages: [{ role: 'user', content: prompt }],
